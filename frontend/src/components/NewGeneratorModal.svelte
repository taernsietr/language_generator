...
{#if ($currentlyDisplaying == "NewGeneratorModal")}
    ...
    <p class="p-2 m-2 opacity-100">
        Type in a name for the new generator and click the Confirm button, or click the <span class="text-red">X</span> to cancel.
    </p>
    <input
        class="bg-bg2 p-2 m-2 text-center no-spinner text-yellow placeholder:opacity-50"
        type="text"
        placeholder="generator name"
        bind:value={generatorNameInput}
    />
